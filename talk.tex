...
\begin{frame}{The problem in a nutshell}
  ...
  \begin{center}
    \input{pics/chemicalPlantAutomaton}
  \end{center}
  }%
...
\begin{frame}{Flowpipe construction \& reach tree}
  ...
  \begin{result}{In a nutshell}{\linewidth}
    ...
    \begin{itemize}
      ...
      \item In case several non-stochastic jumps are enabled: non-deterministic choice ($\rightarrow$ reach tree)
    \end{itemize}
  \end{result}
  }
\end{frame}
...
\begin{minipage}{.7\textwidth}
  ...
  \cb{Approach}
  \begin{itemize}
    \item Determine sets of reachable states that satisfy the goal-condition (location \& state sets)
...
\begin{frame}{Exemplary case study}
  ...
  Results
  \begin{itemize}
    \item Compute maximal optimal probabilities for prophetic and non-prophetic schedulers
    \item Results comparable to existing approaches, non-prophetic only
  \end{itemize}
  }%
\end{frame}
